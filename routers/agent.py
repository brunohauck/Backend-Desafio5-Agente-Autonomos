...
# Local por padrão; em produção (Render), defina BACKEND_BASE_URL
BASE_URL = os.getenv("BACKEND_BASE_URL") or os.getenv("RENDER_EXTERNAL_URL") or "http://localhost:8000"
...
def ask_agent(req: AgentRequest):
    ...
    if not csv.exists():
        raise HTTPException(404, "Dataset não encontrado no servidor.")
    ...
    plot_path = plot_url = None

    if isinstance(meta, dict) and "plot" in meta and BASE_URL:
        plot = meta["plot"]
        try:
            if plot.get("type") == "hist_amount":
                r = requests.get(f"{BASE_URL}/plot/amount_hist/{req.dataset}",
                                 params={"bins": plot.get("bins", 50), "log": plot.get("log", True)}, timeout=180)
                if r.ok: j = r.json(); plot_path, plot_url = j.get("plot_path"), j.get("plot_url")

            elif plot.get("type") == "timeseries":
                r = requests.get(f"{BASE_URL}/plot/time_series/{req.dataset}",
                                 params={"bins": plot.get("bins", 120)}, timeout=180)
                if r.ok: j = r.json(); plot_path, plot_url = j.get("plot_path"), j.get("plot_url")

            elif plot.get("type") == "corr_heatmap":
                r = requests.get(f"{BASE_URL}/plot/corr_heatmap/{req.dataset}",
                                 params={"sample_rows": plot.get("sample_rows", 50000)}, timeout=240)
                if r.ok: j = r.json(); plot_path, plot_url = j.get("plot_path"), j.get("plot_url")

            elif plot.get("type") == "box_amount_by_class":
                r = requests.get(f"{BASE_URL}/plot/box_amount_by_class/{req.dataset}",
                                 params={"max_per_class": plot.get("max_per_class", 20000)}, timeout=240)
                if r.ok: j = r.json(); plot_path, plot_url = j.get("plot_path"), j.get("plot_url")

            elif plot.get("type") == "scatter":
                r = requests.get(f"{BASE_URL}/plot/scatter_pca/{req.dataset}",
                                 params={"x": plot.get("x","V1"), "y": plot.get("y","V2"),
                                         "sample_rows": plot.get("sample_rows", 50000)}, timeout=240)
                if r.ok: j = r.json(); plot_path, plot_url = j.get("plot_path"), j.get("plot_url")
    ...
    if plot_url: details["plot_url"] = plot_url
    if plot_path: details["plot_path"] = plot_path
